...
any of it.** Every command here is `git` or a recursive delete, all of which the
coordinator guard denies (default-deny: only relay-targeted `curl`/`wget`,
markdown writes, and inert inspection are permitted).
...
This repo checkout **is** the live deployment — the server watches its own source, and `public/` is bind-mounted and served straight off disk.
...
- **Verify a deploy at `/`, never `/index.html`** — the latter falls through to a 46-byte JSON 404 that looks exactly like a failed ship.
  ...
  ```
- **Check for a link before any recursive delete under a worktree.** Some worktrees have `node_modules` linked into this repo's real one, so a recursive delete follows the link and destroys the live app's dependencies. The check and the safe unlink are OS-specific: `windows-legacy.md`.
