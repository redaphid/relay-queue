...
- **`/health` counts every tab ever made; `GET /conversations` lists only live ones.** Both correct, and they will not match - `conversationCounts: {total, live, archived}` reconciles them. A pending task whose `conversationId` is absent from the list is in an **archived** tab, not a deleted one.
...
- **The claim field is `by`; `agent`, `author` and `claimedBy` are aliases** recording the same holder, `by` wins, and a bodyless claim is still deliberately allowed and recorded as anonymous. **Older transcripts warn that anything but `by` returns 200 with `claimedBy:null`** - true until 2026-08-27, not true now. Trust this line over the transcript. `tools/claim-identity-selftest.js`.
- **Single-resource replies carry both shapes: the record bare, and under its canonical key** (`task`, `conversation`, `message`); collections stay `{count, tasks}`. So `body.claimedBy` and `body.task.claimedBy` are both right. Only the bare form existed before 2026-08-31, and a guard reading `body.task.claimedBy` got `undefined` on four SUCCESSFUL claims, concluded they failed, and left all four `claimed` with `result:null`. **`undefined` from a read-back means check the shape before you conclude the write failed.**
- **`result` DOES have an ownership guard, and older transcripts say it does not.** Observed 2026-09-01: posting a result as a different agent is refused `409 "<holder>" holds this task, not "<you>"`. That matters most when you inherit an **orphaned claim** — an agent stopped mid-flight leaves a task `claimed` with `result:null`, which is invisible to future polls and looks answered forever. **To clear one, omit `by` entirely**; ownership is only checked when `by` is present and mismatches. Sign yourself in the result body instead. Do not post under the dead agent's name — relay authenticates nothing, so impersonation succeeds and lands in the permanent record. The deadman banner renders `claimedBy || 'an agent'`, so the stuck task also reports namelessly.
...
- **`GET /tasks` rows carry `unstaffable` - read it before concluding autoseat is broken.** `null` means nothing permanent blocks a seat; otherwise `{code, why}` (tab archived, tab stopped, or `from` is not one of his clients). `/health` totals it. **A nonzero `pending` is not an outage**: on 2026-08-31 a 7-task, 8-day backlog was refused correctly by design and restarting the healthy dispatcher was the obvious wrong move. **`unstaffable:0` beside a real backlog means selection is fine and the fault is downstream** - dispatched and died.
