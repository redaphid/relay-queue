...
- **While you are running, watch your own tab and claim your own tasks** (unless you are an `auto-…` coordinator started by autoseat: it holds the watch for you and hands you each new message as a new turn — do not arm a Monitor or poll loop, just end your turn) — don't wait for a router to relay each message, which costs a coordinator round-trip every time. See **Watch, don't poll**; after any reconnect, poll `GET /tasks?conversation=<yours>&status=pending`, because a stream only carries events forward.
- **A seat means a live process, not an assignment. If your process is ending, release the seat — even mid-topic.** A turn-scoped agent cannot hold a stream across turns, so "I'll keep the seat and keep watching" is a claim you cannot honour: `agentState.listeners` is the server counting live SSE subscribers on *your* conversation, and it will read `0` the moment you stop. Leaving your name there produces a phantom seat that `seatUnwatched` flags in 2 minutes and that autoseat will not replace, because the chair looks taken. **An honestly empty seat gets refilled; a falsely occupied one strands the tab.** Vacating often is correct and cheap — say why (`agentLeftReason`) and let the next dispatch pick it up. **Exception, since 2026-09-17: an `auto-…` coordinator does NOT release its seat or exit.** Its process is long-lived, autoseat holds the SSE listener for exactly as long as that process lives, and autoseat releases the seat itself when it retires it. Answer, then end the turn.
...
**Auto-seated coordinators are long-lived (since 2026-09-17).** One process per tab, fed each later human message as a new turn on stdin; autoseat holds the tab's SSE watch, retires the process after 30 idle minutes (releasing the seat, keeping the session), evicts the least recently active idle one when 6 are live, and revives a dead or retired one with `claude --resume` on the tab's next message. **If you are one: take the seat once, keep your name, answer, END THE TURN. Never release the seat, never exit, never arm a watcher.**
...
- **The claim field is `by`, not `agent` — and getting it wrong fails silently and unrecoverably.** `POST /tasks/<id>/claim {"agent":"Name"}` returns **200** and sets `status:"claimed"` with **`claimedBy: null`**: `claimTask()` reads `body.by` only (`const by = typeof body.by === 'string' && body.by ? body.by : null`), and it cannot reject a missing one because a bare bodyless claim is deliberately still supported. The task is now held by nobody for the full 15-minute `CLAIM_LEASE_MS`, and **you cannot fix your own mistake** — re-claiming correctly with `{"by":"Name"}` returns `409 task is already claimed` reporting `claimedBy: null`. Nothing else flags it: `progress` still returns 200 (it allows an unheld task by design).
  ...
  **`result` DOES now have an ownership guard, and this line used to say it did not.** Observed 2026-09-01: posting a result as a different agent is refused `409 "<holder>" holds this task, not "<you>"`. That matters most when you inherit an **orphaned claim** — an agent stopped mid-flight leaves a task `claimed` with `result:null`, which is invisible to future polls and looks answered forever. **To clear one, omit `by` entirely**; ownership is only checked when `by` is present and mismatches. Sign yourself in the result body instead. Do not post under the dead agent's name — relay authenticates nothing, so impersonation succeeds and lands in the permanent record. The deadman banner renders `claimedBy || 'an agent'`, so the stuck task also reports namelessly. **The trap is that `agent` is the correct field for `POST /conversations/<id>` and for `/activity`, so the wrong body reads right.** Two different agents hit this independently on 2026-08-26; verified end-to-end the same day against a throwaway instance (own `DATA_DIR`, own port), not inferred from source.
...
- **Never post the same text as both a message and a task `result`.** A result is emitted into the thread as its own `role:agent` entry (`<taskId>:r`), so duplicating it puts one answer in the thread twice — and the page reads entries aloud, so he *hears* it twice and gets two buzzes. On 2026-09-01 one tab held 15 byte-identical pairs, and one answer that closed two tasks was read three times. **Put the substance in the `result`; keep any accompanying message to a short, different line.** If the work has no task, a message is the right and only home for it.
...
**This skill and the guard live in the relay-queue repo, and neither depends on cwd.** A seat runs in its tab's folder (`~/<conversation.path>`, or the tab's own worktree under `~/Worktrees/` when that folder is in a git repo), so `tools/autoseat.js` injects both into every spawn: `--settings=<repo>/src/claude-config/settings.json` registers the guard (`src/claude-config/hooks/coordinator-guard.js`), and `--add-dir=<repo>` loads this skill. autoseat refuses to start if either is missing or the registration names no guard. **Move the guard and its registration in the same commit** - the registration names its absolute path, and a seat whose hook cannot run is silently unguarded. A session started by hand in the repo is **not** guarded (owner's choice): coordinator mode is a session started by relay. Details: `references/autoseat.md`.
